...
function PremiumStatCard({
  ...
  return (
    <div className="group relative bg-white rounded-none p-6 shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden">
      <div
...
function SystemStatusIndicator({ status }: { status: 'ok' | 'error' }) {
  ...
  return (
    <span className="flex items-center gap-2">
      <span
        className={`inline-block w-2.5 h-2.5 rounded-full ${isOk ? 'bg-green-500 animate-pulse' : 'bg-red-500'
          }`}
      />
      <span
        className={`text-xs font-black uppercase tracking-widest ${isOk ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50'
...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="flex flex-col bg-[#f8fafc] selection:bg-primary/10 min-h-full">
      <main
        className={`flex-1 w-full px-6 pt-8 pb-10 space-y-8 transition-all duration-700 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
          }`}
      >
        {/* 페이지 제목 섹션 */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-gray-200 pb-8">
          <div>
            ...
            <p className="mt-2 text-gray-500 font-medium">
              KATC 유사호출부호 경고시스템 데이터 및 서비스 현황
            </p>
          </div>
        </div>
        ...
        {data && (
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              {/* 최근 로그인 */}
              <div className="lg:col-span-2 bg-white rounded-none shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="space-y-6">
                <div className="bg-white rounded-none shadow-sm border border-gray-100 p-8">
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-col gap-2 p-5 rounded-none bg-gray-50/50 border border-gray-100 group hover:border-emerald-200 transition-colors">
                  ...
                  <div className="mt-8 p-6 rounded-none bg-navy text-white relative overflow-hidden group">
